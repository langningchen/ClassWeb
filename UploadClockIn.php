...
$InputDetail = "";
$DatabaseQuery = $Database->prepare("SELECT * FROM ClockInUploadList WHERE ClockInID=? and UploadUID=?");
$DatabaseQuery->bind_param("ii", $_GET["ClockInID"], $_SESSION["UID"]);
$DatabaseQuery->execute();
$Result = $DatabaseQuery->get_result();
if ($Result->num_rows == 0) {
...
if (isset($_FILES["UploadFile"])) {
    if ($_FILES["UploadFile"]["error"] != 0) {
        ...
        $DatabaseQuery = $Database->prepare("INSERT INTO ClockInUploadFileList(ClockInID, UploadUID, UploadTime, FileName, FileType, FileSize) VALUES (?, ?, current_timestamp(), ?, ?, ?)");
        $DatabaseQuery->bind_param("iisss", $_GET["ClockInID"], $_SESSION["UID"], $_FILES["UploadFile"]["name"], $_FILES["UploadFile"]["type"], $_FILES["UploadFile"]["size"]);
        $DatabaseQuery->execute();
        move_uploaded_file($_FILES["UploadFile"]["tmp_name"], "ClockInUploadFile/" . $DatabaseQuery->insert_id . "_" . $_GET["ClockInID"] . "_" . $_SESSION["UID"] . "_" . $_FILES["UploadFile"]["name"]);
    }
}
if (isset($_POST["Delete"]) && isset($_POST["ClockInUploadFileID"])) {
    $DatabaseQuery = $Database->prepare("SELECT filename FROM ClockInUploadFileList WHERE ClockInUploadFileID=?");
    $DatabaseQuery->bind_param("i", $_POST["ClockInUploadFileID"]);
    $DatabaseQuery->execute();
    $RowData = $DatabaseQuery->get_result()->fetch_array(MYSQLI_NUM);
    $DatabaseQuery = $Database->prepare("DELETE FROM ClockInUploadFileList WHERE ClockInUploadFileID=?");
    $DatabaseQuery->bind_param("i", $_POST["ClockInUploadFileID"]);
    $DatabaseQuery->execute();
...
if (isset($_POST["Submit"]) && isset($_POST["Data"])) {
    $DatabaseQuery = $Database->prepare("SELECT ClockInUploadFileID FROM ClockInUploadFileList WHERE UploadUID=? AND ClockInID=?");
    $DatabaseQuery->bind_param("ii", $_SESSION["UID"], $_GET["ClockInID"]);
    $DatabaseQuery->execute();
    $Result = $DatabaseQuery->get_result();
    $FileList = "";
    ...
    if ($FileList == "" && $InputDetail == "") {
        ...
        $ThisStatus = 0;
        $DatabaseQuery = $Database->prepare("SELECT Status FROM ClockInUploadList WHERE ClockInID=? and UploadUID=?");
        $DatabaseQuery->bind_param("ii", $_GET["ClockInID"], $_SESSION["UID"]);
        $DatabaseQuery->execute();
        $Result = $DatabaseQuery->get_result();
        if ($Result->num_rows == 0) {
            ...
            $DatabaseQuery = $Database->prepare("DELETE FROM ClockInUploadList WHERE ClockInID=? and UploadUID=?");
            $DatabaseQuery->bind_param("ii", $_GET["ClockInID"], $_SESSION["UID"]);
            $DatabaseQuery->execute();
        }
        $DatabaseQuery = $Database->prepare("INSERT INTO ClockInUploadList(ClockInID, UploadUID, Data, FileList, UploadDate, UploadTime, Status) VALUES (?, ?, ?, ?, ?, current_timestamp(), ?)");
        $DatabaseQuery->bind_param("iisssi", $_GET["ClockInID"], $_SESSION["UID"], $InputDetail, $FileList, $_GET["UploadDate"], $ThisStatus);
        $DatabaseQuery->execute();
        $DatabaseQuery = $Database->prepare("SELECT ClassID FROM ClockInList WHERE ClockInID=?");
        $DatabaseQuery->bind_param("i", $_GET["ClockInID"]);
        $DatabaseQuery->execute();
        $RowData = $DatabaseQuery->get_result()->fetch_array(MYSQLI_NUM);
        echo "<script>window.location=\"ClockIns.php?ClassID=" . $RowData[0] . "\"</script>";
        die();
    }
}
if (isset($_POST["Rename"]) && isset($_POST["ClockInUploadFileID"]) && isset($_POST["AfterFileName"])) {
    $DatabaseQuery = $Database->prepare("SELECT filename FROM ClockInUploadFileList WHERE ClockInUploadFileID=?");
    $DatabaseQuery->bind_param("s", $_POST["ClockInUploadFileID"]);
    $DatabaseQuery->execute();
    $RowData = $DatabaseQuery->get_result()->fetch_array(MYSQLI_NUM);
    if (file_exists("ClockInUploadFile/" . $_POST["ClockInUploadFileID"] . "_" . $_GET["ClockInID"] . "_" . $_SESSION["UID"] . "_" . $RowData[0])) {
        ...
        $DatabaseQuery = $Database->prepare("UPDATE ClockInUploadFileList SET filename=? WHERE ClockInUploadFileID=?");
        $DatabaseQuery->bind_param("si", $_POST["AfterFileName"], $_POST["ClockInUploadFileID"]);
        $DatabaseQuery->execute();
    } else {
...
echo "<br />";
$DatabaseQuery = $Database->prepare("SELECT * FROM ClockInUploadFileList WHERE UploadUID=? AND ClockInID=?");
$DatabaseQuery->bind_param("ii", $_SESSION["UID"], $_GET["ClockInID"]);
$DatabaseQuery->execute();
$Result = $DatabaseQuery->get_result();
echo "<table>";
